...
def load_emoji():
    smile_emoji = cv2.imread('project/emoji_prj/mac/smile.png', cv2.IMREAD_UNCHANGED)
    ...
    # 이모티콘 이미지 읽기
    neutral_emoji = cv2.imread('project/emoji_prj/mac/neutral.png', cv2.IMREAD_UNCHANGED)
...
def model_load():
    ...
    # pre trained model
    model.load_state_dict(torch.load("project/emoji_prj/mac/best_model.pt"))
    model.to(device)
    return model
...
def webcam():
    ...
    #얼굴 인식 캐스케이드 파일 읽는다
    face_cascade = cv2.CascadeClassifier('project/emoji_prj/mac/haarcascade_frontalface_alt.xml')
    count = 0
